...
resource "aws_s3_bucket_policy" "lb_log_delivery" {
  count = var.policy == "" && var.attach_lb_log_delivery_policy ? 1 : 0

  bucket = aws_s3_bucket.bucket[0].id
  policy = templatefile("${path.module}/templates/lb_log_delivery.json", { aws_account_id = data.aws_caller_identity.current.account_id, bucket_name = var.bucket_name, lb_service_account = data.aws_elb_service_account.account.0.arn })
}
...
resource "aws_s3_bucket_public_access_block" "bucket" {
  ...
  bucket = var.policy != "" ? aws_s3_bucket_policy.bucket[0].id : aws_s3_bucket.bucket[0].id
