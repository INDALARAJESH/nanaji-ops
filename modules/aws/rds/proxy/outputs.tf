...
output "proxy_endpoint_default" {
  description = "The endpoint that you can use to connect to the proxy"
  value       = aws_db_proxy.this.endpoint
}

output "proxy_endpoint_rw" {
  description = "The (RW) endpoint that you can use to connect to the proxy (optional)"
  value       = element(concat(aws_db_proxy_endpoint.rw.*.endpoint, [""]), 0)
}

output "proxy_endpoint_ro" {
  description = "The (RO) endpoint that you can use to connect to the proxy (optional)"
  value       = element(concat(aws_db_proxy_endpoint.ro.*.endpoint, [""]), 0)
}
