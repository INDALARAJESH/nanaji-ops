# Stock Component
#
# Lambda and SQS configuration for handling stock events
module "stock" {
  source                   = "./sqs_lambda"
  service                  = local.service
  env                      = local.env
  sqs_queue_name           = "stock"
  handler_name             = "stock"
  ...
  environment_variables = merge(
    {
      ...
      DD_LAMBDA_HANDLER              = var.webhook_stock_lambda_command
...
resource "aws_iam_role_policy_attachment" "stock_policy_attachment_dynamo_read" {
  role       = module.stock.lambda_handler_role_name
  policy_arn = aws_iam_policy.dynamodb_read.arn
}

resource "aws_iam_role_policy_attachment" "stock_policy_attachment_dynamo_write" {
  role       = module.stock.lambda_handler_role_name
  policy_arn = aws_iam_policy.dynamodb_write.arn
}

resource "aws_iam_role_policy_attachment" "stock_policy_attachment_ssm_param" {
  role       = module.stock.lambda_handler_role_name
  policy_arn = aws_iam_policy.ssm_param.arn
}

resource "aws_iam_role_policy_attachment" "stock_policy_attachment_kms_use" {
  role       = module.stock.lambda_handler_role_name
  policy_arn = aws_iam_policy.kms_use.arn
}

# https://docs.aws.amazon.com/lambda/latest/dg/configuration-vpc.html#vpc-permissions
resource "aws_iam_role_policy_attachment" "stock_lambda_vpc" {
  role       = module.stock.lambda_handler_role_name
