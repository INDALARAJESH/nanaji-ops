module "post_authorization_url" {
  source = "./api_lambda"

  service                = local.service
  env                    = local.env
  api_lambda_name        = "post-authorization-url"
  ...
  resource_path = "POST/v1/auth/authorization-url"

  environment_variables = merge(
    {
      DYNAMODB_TABLE       = aws_ssm_parameter.dynamodb_table.value
      DYNAMODB_URL         = aws_ssm_parameter.dynamodb_url.value
      DD_LAMBDA_HANDLER    = var.api_post_authorization_url_lambda_command
...
resource "aws_iam_role_policy_attachment" "post_authorization_url_policy_attachment_dynamo_read" {
  role       = module.post_authorization_url.api_lambda_role_name
