module "get_restaurant_errors" {
  source = "./api_lambda"

  service                = local.service
  env                    = local.env
  api_lambda_name        = "get-restaurant-errors"
  ...
  resource_path = "GET/v1/restaurants/{restaurant_id}/integration-errors"

  environment_variables = merge(
    {
      DYNAMODB_TABLE       = aws_ssm_parameter.dynamodb_table.value
      DYNAMODB_URL         = aws_ssm_parameter.dynamodb_url.value
      DD_LAMBDA_HANDLER    = var.api_get_restaurant_errors_lambda_command
...
resource "aws_iam_role_policy_attachment" "get_restaurant_errors_policy_attachment_dynamo_read" {
  role       = module.get_restaurant_errors.api_lambda_role_name
