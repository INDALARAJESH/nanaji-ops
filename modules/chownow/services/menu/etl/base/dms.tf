...
module "menu_etl_dms" {
  ...
  table_mappings                  = templatefile("${path.module}/templates/menu-table-mapping.json", { schema_name = "menu" })

  repl_inst_multi_az       = false
  repl_inst_instance_class = "dms.r4.large"
  repl_inst_engine_version = "3.4.7"

  source_database_name               = var.database_name
  source_engine_name                 = "mysql"
  source_password                    = random_string.secret.result
  source_port                        = data.aws_rds_cluster.dms.port
  source_server_name                 = data.aws_rds_cluster.dms.endpoint
