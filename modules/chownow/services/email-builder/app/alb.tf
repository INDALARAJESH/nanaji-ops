...
module "public_alb" {
  ...
  cname_subdomain_alb = var.service
  env                 = var.env
  service             = var.service
  vpc_id              = data.aws_vpc.selected.id

  security_group_ids = [data.aws_security_group.ingress_vpn_allow.id, module.alb_sg.id]
}

# Target group for Email Builder instance(s) and responds on TCP 80
module "public_tg" {
...
# Listener rule which matches on the host header email-builder.ENV.svpn.chownow.com from
