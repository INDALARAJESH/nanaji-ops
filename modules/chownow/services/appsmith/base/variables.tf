...
locals {
  ...
  host_header = local.env == "ncp" ? ["central.${var.domain}", "${var.service}.${local.dns_zone}"] : ["${var.service}.${local.dns_zone}"]

  mongodb_keypair_pub_key = file("${path.module}/files/keypair-${local.env}.pub")
  common_tags = {
