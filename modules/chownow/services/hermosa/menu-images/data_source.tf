...
data "aws_iam_policy_document" "menuimages_s3_read_cloudflare_only" {
  statement {
    sid       = "AllowGetObjectCloudflare"
    actions   = ["s3:GetObject"]
    effect    = "Allow"
    resources = ["${module.cn_menuimages.bucket_arn}/*"]
...
data "aws_iam_policy_document" "menuimages_s3_policy_document" {
  statement {
    sid = "menuimagess3access"
    ...
    resources = [
      module.cn_menuimages.bucket_arn,
      "${module.cn_menuimages.bucket_arn}/*",
    ]
  }
}
